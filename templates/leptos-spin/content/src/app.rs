                ...
                if let Some(window) = window() {
                    if let Ok(Some(storage)) = window.local_storage() {
                        if let Ok(Some(cached_count_str)) =
                            storage.get_item("{{project-name | snake_case}}_count")
                        {
                    ...
                    if let Some(window) = window() {
                        if let Ok(Some(storage)) = window.local_storage() {
                            let _ = storage.set_item(
                                "{{project-name | snake_case}}_count",
    ...
    let on_click = move |_| {
        ...
        #[cfg(feature = "hydrate")]
        {
            if let Some(window) = window() {
                if let Ok(Some(storage)) = window.local_storage() {
                    let _ = storage
                        .set_item("{{project-name | snake_case}}_count", &new_count.to_string());
                }
            }
        }
...
pub async fn get_count() -> Result<u32, ServerFnError<String>> {
    ...
    match store.get_json::<u32>("{{project-name | snake_case}}_count").await {
        Ok(Some(count)) => {
...
pub async fn increment_count() -> Result<(), ServerFnError<String>> {
    ...
    // Get current count
    let current_count = match store.get_json::<u32>("{{project-name | snake_case}}_count").await
    {
        Ok(Some(count)) => count,
        Ok(None) => 0,
        Err(_) => 0,
    ...
    store
        .set_json("{{project-name | snake_case}}_count", &new_count)
        .await
